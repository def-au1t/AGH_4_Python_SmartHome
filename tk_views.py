...
class WindowsManager(object):
    ...
    def tk_register_view(self, container=None, info=None):
        ...
        button_submit = ttk.Button(container, text="Zarejestruj się",
                                   command=lambda: self.main.try_register(input_username, input_password,
                                                   input_password2))
    ...
    def tk_register_code_view(self, container=None, info=None, username=None, password=None, key=None):
        ...
        label_password = ttk.Label(container, text="Zeskanuj kod QR na urządzeniu mobilnym i podaj wygenerowane hasło:",
                                   style="descriptionBold.Label")
    ...
    def tk_start_view(self, container=None):
        ...
        button_login = ttk.Button(container, text="Zaloguj się", command=lambda: self.tk_login_view(self.container)).grid(row=2, column=1)
        button_register = ttk.Button(container, text="Zarejestruj się",
                                     command=lambda: self.tk_register_view(self.container)).grid(row=2, column=2)
        self.container = container
    ...
    def tk_login_view(self, container=None, info=None):
        ...
        button_back = ttk.Button(container, text="Powrót", command=lambda:self.tk_start_view()).grid()
        self.container = container
    ...
    def tk_login_code_view(self, container=None, info=None, username=None, key=None):
        ...
        if info:
            ...
            label1.grid(pady=5)
        button_submit = ttk.Button(container, text="Potwierdź",
                                   command=lambda: self.main.try_login_code(username, key, input_code))
    ...
    def tk_room_view(self, room_id, container=None):
        ...
        assert room
        label1 = ttk.Label(container, text=room['name'], style='roomName.Label').grid(column=0, pady=10, sticky="")
        ...
        for device_id in range(len(room['devices'])):
            ...
            if 'status' in device['settings']:
                ...
                controls_device[device_id]['button_submit'] = ttk.Button(device_container, text=device['settings']['status'],
                                           command=lambda dev_id = device_id, r_id = room_id: self.main.switch_device(dev_id, r_id, controls_device[dev_id]['button_submit']))
                controls_device[device_id]['button_submit'].grid(column=0, row=1)
            ...
            if 'props' in device['settings']:
                ...
                if 'prop' in device['settings']:
                    ...
                    if len(props) <= int(current):
                        print("nieprawidłowa wartośś opcji inteligentnego obiektu")
                        current = 0
                else:
                    current = 0
            ...
            sep = ttk.Separator(container).grid(column=0, row=2 * device_id + 3, pady=5, sticky="ew")
